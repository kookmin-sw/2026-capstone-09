    ...
    public void handle(final String payload) {
        try {
            ...
            if (response.isSubSummary()) {
                ...
                completedTask = aiTaskService.complete(response.jobId(), summary, mermaidCode);
            } else {
                String result = response.result().asText();
                completedTask = aiTaskService.complete(response.jobId(), result, null);
            }
